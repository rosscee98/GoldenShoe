...
function ProductScreen(props) {
  ...
  const anySizesAvailable =
    product && product.sizesAvailable && product.sizesAvailable.length
  ...
  const currentlyFavourited = favouriteItems.find(
    (x) => x.product === props.match.params.id,
  )
  ...
  return loading ? (
    ...
    <Container className="content-container px-4 pt-0 pb-3" fluid>
      ...
      <div
        id="productWindow"
        className="bg-dark-grey px-3 py-2 mt-0 round-edge-bottom"
      >
        ...
        <div
          id="productDesc"
          className="rounded px-3 py-3 my-2 bg-white shadow-sm"
        >
          <h3>{product.name}</h3>
          ...
          {showBasketAlert && (
            <Alert
              className="mt-3 mb-0"
              variant="success"
              onClose={() => setShowBasketAlert(false)}
              dismissible
            >
              Added to basket!
            </Alert>
          ...
          {anySizesAvailable ? (
            <Form.Group className="mt-2">
              ...
              <Form.Control
                as="select"
                onChange={(e) =>
                  (sizeSelectionWithCount.current = e.target.value)
                }
              >
          ...
          <Row className="justify-content-between px-3">
            {anySizesAvailable ? (
              <Button
                onClick={() =>
                  handleAddToBasket(product._id, sizeSelectionWithCount.current)
                }
              >
                Add to cart
              </Button>
